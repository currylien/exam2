...
void acc(Arguments *in, Reply *out);                   //RPC for collect acc data
void angle_detection(Arguments *in, Reply *out);       // RPC for feature
...
int feature1_1[100] = {0};                     // arrays for feature data
...
void publish_message(MQTT::Client<MQTTNetwork, Countdown>* client) {  // MQTT publish
  if (mode == 1) {
    mode = 0;
...
int main(int argc, char* argv[]) {
  ...
  while (true) {                                     //RPC loop
    ...
    while (mode == 2) {                        // mode for getting feature
      int get1;
      int get2;
      //if (inital = 1) {
      ...
      for (int i = 0; i < 99; i++) {            // if change in z direction > 3
         int gap;
...
void acc (Arguments *in, Reply *out)   //RPC function for acc
{
...
void acc_collect()   // thread function for acc
{
  mode = 1;
  myled1 = 1;
  uLCD.cls();
...
void angle_det(void)    //thread function for feature
{
  //inital = 0;
  mode = 2;
  /*myled2 = 1;
...
void angle_detection (Arguments *in, Reply *out)  // RPC function for feature
{
